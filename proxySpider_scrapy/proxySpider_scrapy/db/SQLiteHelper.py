...
dbPath='/dick/PycharmProject/IPProxys/data/proxy.db'
from proxySpider_scrapy.db.SqlHelper import SqlHelper
...
class SqliteHelper(SqlHelper):
    ...
    def __init__(self):
        ...
        self.database = sqlite3.connect(dbPath,check_same_thread=False)
    ...
    def insert(self,tableName,value):
        # Dick: if ip/port exist, then update, otherwise insert


        self.cursor.execute("SELECT COUNT(*) FROM %s where ip = \'%s\' and port = %s" % (tableName , value['ip'],value['port']))
        curCnt = self.cursor.fetchone()[0]
        print "curCnt=",curCnt
        if (curCnt > 0) :
            ...
            # print proxy
            self.cursor.execute(
                "UPDATE %s set types=%s,protocol =%s, country = \'%s\', area=\'%s\' ,speed = %s where ip=\'%s\' and port = %s" %
                (tableName,
                value['type'], value['protocol'], value['country'], value['area'],
                value['speed'],
                value['ip'], value['port']))
            #UPDATE proxys set ip='10.0.0.0',port = '80',types='1',protocol = '' , country = '', area='' ,speed = '' where ip='112.112.70.118' and port = '80'
        else:
            proxy = [value['ip'], value['port'], value['type'], value['protocol'], value['country'], value['area'],
                     value['speed']]
            # print proxy
